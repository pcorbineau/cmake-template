...
// ---------------------------------------------------------------------------
static void xdg_wm_base_ping(void* /*data*/, xdg_wm_base* base, u32 serial)
{
    ...
    u32          version)
{
    ...
    if (std::string_view{ interface } == wl_compositor_interface.name) {
        ...
        state->xdg_wm = static_cast<xdg_wm_base*>(
...
auto WaylandTraits::create(WindowConfig cfg) -> Handle
{
    ...
    xdg_toplevel_set_min_size(
        state->toplevel,
        static_cast<i32>(cfg.width.get()),
        static_cast<i32>(cfg.height.get()));
...
auto WaylandTraits::destroy(Handle& handle) -> void
{
    ...
    if (s.toplevel != nullptr) { xdg_toplevel_destroy(s.toplevel);  s.toplevel = nullptr; }
    if (s.xdg_surf != nullptr) { xdg_surface_destroy(s.xdg_surf);  s.xdg_surf = nullptr; }
    if (s.surface   != nullptr) { wl_surface_destroy(s.surface);    s.surface  = nullptr; }
    if (s.xdg_wm    != nullptr) { xdg_wm_base_destroy(s.xdg_wm);   s.xdg_wm   = nullptr; }
    if (s.compositor!= nullptr) { wl_compositor_destroy(s.compositor); s.compositor = nullptr; }
    if (s.registry  != nullptr) { wl_registry_destroy(s.registry);  s.registry  = nullptr; }
    if (s.display   != nullptr) { wl_display_disconnect(s.display); s.display   = nullptr; }
